...
function SnippetsPage() {
  ...
  // state for selected language filter
  const [selectedLanguage, setSelectedLanguage] = useState<string | null>(null); // null means no filter, show all languages
  // state for selected view filter
  ...
  const languages = [...new Set(snippets.map((s) => s.language))]; // use set so not repeated
  const popularLanguages = languages.slice(0, 5); // get top 5 languages

  const filteredSnippets = snippets.filter((snippet) => {
    const matchesSearch =
      snippet.title.toLowerCase().includes(searchQuery.toLowerCase()) || // title matches
      snippet.language.toLowerCase().includes(searchQuery.toLowerCase()) || // language matches
      snippet.userName.toLowerCase().includes(searchQuery.toLowerCase());  // author matches

    // if no language selected, match all, otherwise match selected language with current
    const matchesLanguage = !selectedLanguage || snippet.language === selectedLanguage;

    // current snippet must match both search and language filter
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="relative max-w-5xl mx-auto mb-12 space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            {/* Generate filter buttons for popular languages */}
        ...
        <motion.div
          className={`grid gap-6 ${
            view === "grid"
              ? "grid-cols-1 md:grid-cols-2 lg:grid-cols-3" // grid view: put them side-by-side
              : "grid-cols-1 max-w-3xl mx-auto" // list view: stack them and center
          }`}
          layout
        >
